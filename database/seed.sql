-- Sakura Shop DZ — initial data
-- Import AFTER schema.sql.
--
-- Creates one admin account:
--   email:    admin@sakurashop.dz
--   password: ChangeMe123!
-- Log in at /admin/login.php and change this password immediately (create a
-- new admin user with a password you choose, then remove this one, since
-- there is no "change password" screen yet) — or use database/create_admin.php.

USE sakura_shop;

-- INSERT IGNORE: safe to re-run even if this admin account already exists
-- (email is UNIQUE, so a duplicate insert is silently skipped instead of erroring).
INSERT IGNORE INTO users (name, email, phone, password_hash, role) VALUES
('Admin', 'admin@sakurashop.dz', NULL, '$2y$10$VqaXQi/Ox15zlklhNkxeEe4zqupgbcMblqhN2nNAG/YQ6ENfclvjm', 'admin');
